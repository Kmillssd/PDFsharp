...
namespace PdfSharp.Drawing.BarCodes
{
    ...
    public class Code128 : BarCode
    {
        ...
        protected internal override void Render(XGraphics gfx, XBrush brush, XFont font, XPoint position)
        {
            ...
            for (Int32 index = 0; index < this.Text.Length; index++)
                switch (this.Code128Code)
                {
                    case Code128Type.A:
                        if (this.Text[index] < 32)
                            this.Values[index] = (byte)(this.Text[index] + 64);
                        else if ((this.Text[index] >= 32) && (this.Text[index] < 64))
                            this.Values[index] = (byte)(this.Text[index] - 32);
                        else
                            this.Values[index] = (byte)this.Text[index];
                        break;
                    case Code128Type.B:
                        this.Values[index] = (byte)(this.Text[index] - 32);
                        break;
                    case Code128Type.C:
                        if ((this.Text[index] >= '0') && (this.Text[index] <= '9'))
                        {
                            buffer += this.Text[index];
                            if (buffer.Length == 2)
                            {
